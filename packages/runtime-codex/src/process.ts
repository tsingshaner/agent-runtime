...
/**
 * One project-owned app-server process and its native session protocol.
 */
export class CodexProcess implements RuntimeAdapter {
  ...
  private async importLegacyHistory(nativeSessionId: string): Promise<void> {
    ...
    const marker = join(home, 'imported-history', nativeSessionId)
    if (
    ...
    const markOwned = async () => {
      await mkdir(join(home, 'imported-history'), { mode: 0o700, recursive: true })
      await writeFile(marker, '', { mode: 0o600 })
    }
    const pattern = `sessions/**/rollout-*-${nativeSessionId}.jsonl`
    for await (const _ of glob(pattern, { cwd: home })) {
      await markOwned()
      return
    }
    ...
    for await (const path of glob(pattern, { cwd: source })) {
      const file = await realpath(join(source, path))
      const within = relative(source, file)
      if (isAbsolute(within) || within === '..' || within.startsWith(`..${sep}`)) {
        throw new RuntimeError('INVALID_INPUT', 'Native history escapes its source directory')
      }
      const destination = join(home, 'sessions', basename(path))
      await mkdir(join(home, 'sessions'), { mode: 0o700, recursive: true })
