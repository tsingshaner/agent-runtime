...
export class CodexRuntime implements RuntimeAdapter {
  readonly kind = 'codex'
  readonly #options: ReturnType<typeof parseRuntimeOptions>
  readonly #projects = new Map<string, Promise<CodexProcess>>()
  readonly #active = new Map<string, Promise<CodexProcess>>()
  ...
  #project(projectId?: string): Promise<CodexProcess> {
    this.#checkOpen()
  ...
  async #prepareProject(projectId: string): Promise<CodexProcess> {
    const root = join(
